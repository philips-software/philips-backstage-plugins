...
const deprecatePackage = async (options: { package: Package }) => {
  ...
  await fs.writeFile(
    ...
    `# Deprecated\n\nThis package has been moved to the [backstage-community/plugins](https://github.com/philips-software/philips-backstage-plugins) repository. Migrate to using \`${newPackageName}\` instead.\n`,
  );

  // then update package.json
  const packageJsonPath = path.join(options.package.dir, 'package.json');
  const packageJson = await fs.readJson(packageJsonPath);
  packageJson.deprecated = `This package has been moved to the philips-software/philips-backstage-plugins repository. You should migrate to using ${newPackageName} instead.`;
...
export default async (opts: OptionValues) => {
  ...
  await createChangeset({
    ...
    message:
      'These packages have been migrated to the [philips-software/philips-backstage-plugins](https://github.com/philips-software/philips-backstage-plugins) repository.',
  });
